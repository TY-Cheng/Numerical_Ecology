...
rm(list = ls())
load('ANOVAt_Test_per_Genus.RData')
df_Abundancy_Genus <- df_Abundancy_Genus_1
df_Abundancy_Genus <- df_Abundancy_Genus_2
    ...
    df_result <- alply(
        ...
        .fun = function(
            ...
            df_temp <- data.frame(
                ...
                Abundancy = as.numeric(
                    iter_Abundancy_Genus[c(seq_DI, seq_Mock, seq_AeMBR, seq_AnMBR)])
            )
            temp_pval <- c(
